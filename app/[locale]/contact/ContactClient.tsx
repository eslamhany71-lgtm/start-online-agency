...
export default function ContactClient({ dict }: { dict: any }) {
  const [formData, setFormData] = useState({
    name: "", company: "", phone: "", email: "", inquiry: "", subject: "", message: "",
  });
  ...
  return (
    <main className="min-h-screen bg-[#FCFBFA] text-[#111111] flex flex-col items-center justify-start overflow-hidden pt-20">
      ...
      <section className="w-full max-w-[1280px] mx-auto px-6 md:px-8 lg:px-20 min-h-[500px] flex flex-col lg:flex-row items-center justify-between gap-12 py-16 lg:py-20">
        <div className="w-full lg:w-[55%] z-10 flex flex-col text-center lg:text-left rtl:lg:text-right">
          ...
          <h1 className="text-[48px] md:text-[64px] font-extrabold leading-[1.05] tracking-tight mb-6">
            {dict.hero.title1} <span className="text-[#E01E2E]">{dict.hero.titleHighlight}</span>
          </h1>
        ...
        <div className="w-full max-w-[1280px] mx-auto px-6 md:px-8 lg:px-20">
          <div className="flex flex-col lg:flex-row gap-[28px]">
            ...
            <div className="w-full lg:w-[40%] bg-[#141516] text-white rounded-[24px] p-8 md:p-[40px] flex flex-col shadow-xl">
              ...
              <div className="flex flex-col gap-8 flex-1">
                ...
                <a href="mailto:hello@startonline.agency" className="group flex items-start gap-4 hover:-translate-y-1 transition-transform">
                  ...
                  <div>
                    <p className="text-[12px] font-bold text-gray-400 uppercase tracking-wider mb-1">{dict.info.email}</p>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-[12px] font-bold text-gray-400 uppercase tracking-wider mb-1">{dict.info.hoursTitle}</p>
                    <p className="text-[16px] font-medium">{dict.info.hours}</p>
                  </div>
                </div>
              </div>
            ...
            <div className="w-full lg:w-[60%] bg-white border border-[rgba(17,17,17,0.08)] rounded-[24px] p-8 md:p-[44px] shadow-sm">
              ...
              <h2 className="text-[32px] font-bold text-[#111111] mb-8">{dict.form.title}</h2>
              ...
              {isSubmitted ? (
                <div className="text-center py-12 flex flex-col items-center">
                  ...
                  <h3 className="text-[20px] font-bold text-[#111111] mb-2">{dict.form.successTitle}</h3>
                  <p className="text-[#666A73] text-[14px] leading-[1.6] mb-6">
                    {dict.form.successDesc}
                  </p>
                  <button 
                    onClick={() => setIsSubmitted(false)}
                    className="text-[13px] font-bold text-[#E01E2E] hover:text-red-800 transition-colors flex items-center gap-1"
                  >
                    {dict.form.sendAnother} <span className="rtl:rotate-180">→</span>
                  </button>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[12px] font-bold text-[#111111]">{dict.form.name}</label>
                      <input 
                        type="text" required
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[12px] font-bold text-[#111111]">{dict.form.company}</label>
                      <input 
                        type="text" 
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[12px] font-bold text-[#111111]">{dict.form.email}</label>
                      <input 
                        type="email" required
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[12px] font-bold text-[#111111]">{dict.form.phone}</label>
                      <input 
                        type="tel" required dir="ltr"
                        value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value.replace(/[^0-9+]/g, '') })}
                        className="w-full h-[52px] bg-white border border-[rgba(17,17,17,0.08)] rounded-[12px] px-4 text-[14px] text-[#111111] focus:outline-none focus:border-[#E01E2E] focus:ring-2 focus:ring-[#E01E2E]/10 transition-all text-left rtl:text-right" 
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[12px] font-bold text-[#111111]">{dict.form.inquiry}</label>
                      <select 
                      ...
                      >
                        <option value="" disabled>{dict.form.selectPlaceholder}</option>
                        {dict.form.options.map((cat: string) => <option key={cat} value={cat}>{cat}</option>)}
                      </select>
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[12px] font-bold text-[#111111]">{dict.form.subject}</label>
                      <input 
                        type="text" required
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[12px] font-bold text-[#111111]">{dict.form.message}</label>
                    <textarea 
                      required
      ...
      <section className="w-full bg-white py-[96px] lg:py-[120px] border-t border-[rgba(17,17,17,0.04)]">
        <div className="w-full max-w-[800px] mx-auto px-6 md:px-8 text-center">
          <p className="text-[#E01E2E] text-[12px] font-bold uppercase tracking-[0.1em] mb-4">{dict.faq.badge}</p>
          <h2 className="text-[32px] md:text-[38px] font-bold leading-tight mb-16">{dict.faq.title}</h2>
